...
class RootFrame(tk.Frame):
    ...
    def move_window(self, event):
        if self.maximized:
            # Implement snap drag
            if abs(event.x_root - self.drag_start_x) > 10 or abs(event.y_root - self.drag_start_y) > 10:
                # Calculate the ratio of cursor position within the maximized window
                width_ratio = self.xwin / self.parent.winfo_width()
                
                # Restore the window
                self.restore_window()
                
                # Calculate new position based on the ratio
                new_x = event.x_root - int(self.parent.windowSize[0] * width_ratio)
                new_y = event.y_root - self.ywin
                
                # Ensure the window stays within screen bounds
                new_x = max(0, min(new_x, self.parent.winfo_screenwidth() - self.parent.windowSize[0]))
                new_y = max(0, min(new_y, self.parent.winfo_screenheight() - self.parent.windowSize[1]))
                
                self.parent.geometry(f'+{new_x}+{new_y}')
                self.previousPosition = [new_x, new_y]
                
                # Update drag start position
                self.drag_start_x = event.x_root
                self.drag_start_y = event.y_root
                self.xwin = int(self.parent.windowSize[0] * width_ratio)
        else:
